...
import com.skytracks.skytracks.data.CreditCardDao;
...
public class PaymentPageController implements Initializable {
    ...
    void payNow(ActionEvent event) {
        ...
        try {
            ...
            ticketController.setData(reservation, selectedSeats);
...
//        }
//
//        // Iterate over selected seats and process payment for each seat
//        for (Seat seat : selectedSeats) {
//            Reservation reservation = new Reservation(flight, Account.getCurrentUser().getId(), seat, spinnerLuggage.getValue(), spinnerWeight.getValue());
//
//            if (Account.getCurrentUser().hasReservation(flight)) {
//                reservationDao.update(Account.getCurrentUser().getReservation(flight).getId(), reservation);
//            } else {
...
//            TicketPageController ticketController = ticketLoader.getController();
//
//            // Pass the list of selected seats to the ticket controller
//            ticketController.setData(res);
//
//            ApplicationController.navBarController.pushPage(page);
//
//            StackPane content = (StackPane) parent.getScene().lookup("#content");
...
//
    ...
    public void setData(Flight flight, List<Seat> selectedSeats) {
        this.flight = flight;
        this.selectedSeats = selectedSeats;
        this.selectedSeat = selectedSeats.get(0);
        ...
        String classes = "";
        for (Seat seat : selectedSeats) {
            classes +=  seat.getColumn()  + seat.getRow()+", " ;
            lblClassType.setText(classes);
            switch (seat.getType().toLowerCase()) {
                case "first":
            ...
            price += (int) classPrice;
            classPrice = price;
            lblClassPrice.setText(String.valueOf(price));
        }
